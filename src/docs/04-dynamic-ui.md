...
function App() {
  ...
  return (
    <Repeat count={select(items, (i) => i.length)}>
      {(index) => <div>{select(items, (i) => i[index])}</div>}
    </Repeat>
  );
}
```

Or you can use `List` to render items from an array, with a `itemKey` function to allow re-ordering:
